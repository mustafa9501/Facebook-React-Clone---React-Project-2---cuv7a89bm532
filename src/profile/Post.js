        ...
        <>
            ...
            <div className={`${darkTheme && 'dark'}`}>
                    ...
                    <div className={`w-full h-full ${darkTheme && 'dark'}`}>
                        <div className='bg-[#F0F2F5] dark:bg-[#18191A] flex justify-center gap-4 pl-60'>
                            {/* left */}
                            <div className='Left w-1/8 mt-5'>
                                <div className='bg-white dark:bg-[#323436] rounded-lg flex'>
                                    ...
                                    <div className='pl-2 pt-3.5'>
                                        <h2 className='dark:text-white'>You locked your profile</h2>
                                        <p className='font-semibold text-blue-600'>Learn more</p>
                                    </div>
                                </div>
                                <div className='w-full rounded-lg bg-white dark:bg-[#323436] mt-4 pt-2 px-4 pb-4'>
                                    <h2 className='font-bold text-xl pb-2 dark:text-white'>Intro</h2>
                                    {getData.workExperience && getData.workExperience.map((obj, _id) => (
                                        <div key={obj._id} className='flex pt-2'><Icon icon="basil:bag-solid" width="1.5rem" height="1.5rem" style={{ color: '#8C939D' }} />
                                            <div >
                                                <h4 className='text-md text-[#5A5A5A] dark:text-white mt-0.5 pl-5'>Work at {obj.companyName}</h4>
                                            </div>

                                        </div>))}
                                    {getData.address && getData.address.map((obj, _id) => (
                                        <div className='flex'><Icon icon="ic:round-house" width="1.5rem" height="1.5rem" style={{ color: darkTheme ? 'white' : '#8C939D' }} />
                                            <h4 className='text-md text-[#5A5A5A] dark:text-white mt-0.5 pl-5'>{obj.street},  {obj.city}, {obj.country}</h4>
                                        </div>
                                    ))}
                                    <div className='flex'><Icon icon="ic:baseline-email" width="1.4rem" height="1.4rem" style={{ color: darkTheme ? 'white' : '#8C939D' }} />
                                        <h4 className='text-sm text-[#5A5A5A] dark:text-white mt-0.5 pl-5'>{getData.email}</h4>
                                    </div>
                                    <div className='flex'><Icon icon="ph:gender-male-bold" width="1.5rem" height="1.5rem" style={{ color: darkTheme ? 'white' : '#8C939D' }} />
                                        <h4 className='text-md text-[#5A5A5A] dark:text-white mt-0.5 pl-5'>{getData.gender || 'male'}</h4>
                                    </div>
                                    <div className='flex'><Icon icon="carbon:time-filled" width="1.4rem" height="1.4rem" style={{ color: darkTheme ? 'white' : '#8C939D' }} />
                                        <h4 className='text-sm text-[#5A5A5A] dark:text-white mt-0.5 pl-5'>{getData.createdAt}</h4>
                                    </div>
                                </div>

                                <div className='w-full rounded-lg bg-white dark:bg-[#323436] mt-4 px-4 py-4 flex justify-between'>
                            ...
                            <div className='Right w-1/8 mt-5 overflow-y-auto scrollbar' style={{ width: 'calc(80% - 3rem)' }}>
                                <div className='RightContent' style={{ width: 'calc(80% - 3rem)', overflowY: 'auto', maxHeight: 'calc(100vh - 4rem)' }}>
                                    <div className='bg-white dark:bg-[#323436] w-full rounded-lg mb-4'>
                                        ...
                                        <div className='border-b border-gray-300 dark:border-gray-500'></div>
                                        <div className='flex justify-evenly gap-1 pl-6'>
                                            ...
                                            <div className='py-1 w-full pr-5 '>
                                                <button className='flex justify-center gap-2 font-semibold text-[#606770] rounded-lg w-full py-2 hover:bg-[#F0F2F5] dark:hover:bg-[#323436] cursor-not-allowed dark:text-white'>
                                    ...
                                    <div className='w-full mb-4 pr-2'>
                                        {getPost.map((obj) => (
                                            <Card key={obj._id} className="w-full ml-2 rounded-xl mb-3 dark:bg-[#323436]">
                                                <CardHeader className="flex justify-between gap-3.5 dark:bg-[#323436]">
                                                    <div className="py-3 flex">
                                                        <img className="w-10 h-10 rounded-full mt-1" src={getData.profileImage || ProfileImage} alt="Rounded avatar" />
                                                        <div className=''>
                                                            <Typography variant="h4" color="blue-gray" className="px-3 text-lg dark:text-white">
                                                                {getData.name}
                                                            </Typography>
                                                            <h5 className='text-[10px] pl-3.5 dark:text-white'>{getData.createdAt}</h5>
                                                        </div>
                                                    </div>
                                                ...
                                                <CardBody floated={false} className="h-full w-full">
                                                    <p className="pl-6 pr-3 pb-3 dark:text-white">{obj.content}</p>
                                                    <div className="flex justify-center mb-3">
                                                        <img src={obj.images} alt='image' />
                                                    </div>
                                                    <div className="flex justify-between">
                                                        <div className="ml-4 mb-2.5 bg-blue-500 w-5 h-5 rounded-full flex gap-3" onClick={() => likeHandler(obj._id)}><div className="flex mt-1 ml-1"><Icon icon="mdi:like" width="0.8rem" height="0.8rem" style={{ color: darkTheme ? 'white' : 'white' }} /></div>
                                                            <h5 className="mb-2 dark:text-white">{obj.likeCount}</h5>
                                                        </div>
                                                        <div className="mr-4 flex gap-1"><h5 className='dark:text-white'>{obj.commentCount}</h5>
